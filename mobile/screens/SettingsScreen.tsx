	...
	const handleTest = useCallback(async () => {
		...
		try {
			...
			if (response.ok) {
				...
				Alert.alert(
					"Success",
					"Connected to server. API key will be validated on first use.",
				);
			} else {
				Alert.alert("Error", "Server returned an error. Check server status.");
			}
		} catch (error) {
			...
			<ScrollView style={styles.scrollView} keyboardShouldPersistTaps="handled">
							...
							<View style={styles.pickerContainer}>
								{PROVIDERS.map((p) => (
									<TouchableOpacity
										...
										style={[
											styles.pickerItem,
											provider === p.id && styles.pickerItemSelected,
										]}
				...
				<View style={styles.infoSection}>
					...
					<Text style={styles.infoText}>
						API keys are stored securely on your device. Bring your own provider
						key for enhanced speech analysis.
					</Text>
				</View>
			</ScrollView>
		</KeyboardAvoidingView>
