...
export default function SessionCompletionScreen() {
  ...
  const calculateAverageScore = () => {
    ...
    const total = scores.reduce((sum, score) => {
      return (
        sum + score.rhythm_score + score.stress_score + score.pacing_score + score.intonation_score
      );
    }, 0);
  ...
  const calculateCategoryAverage = (category: keyof ExerciseScore) => {
    if (scores.length === 0) return 0;
    const total = scores.reduce((sum, score) => sum + (score[category] as number), 0);
    return Math.round((total / scores.length) * 10) / 10;
  };

  const averageScore = calculateAverageScore();
  const improvement =
    previousAverage !== undefined ? Math.round((averageScore - previousAverage) * 10) / 10 : null;
  ...
  const renderScoreDots = (score: number) => {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      dots.push(
        <View
          key={`dot-${i}`}
          style={[styles.scoreDot, isActive ? styles.scoreDotActive : styles.scoreDotInactive]}
        />,
      );
    }
    return dots;
            ...
            <View style={styles.improvementRow}>
              {improvement > 0 ? (
                <>
                  ...
                  <Text style={styles.improvementText}>Improved by {improvement} points</Text>
                </>
              ) : improvement < 0 ? (
                <>
                  <Ionicons name="arrow-down" size={24} color="#FF9500" />
                  <Text style={styles.improvementText}>Down by {Math.abs(improvement)} points</Text>
                </>
              ) : (
                <>
