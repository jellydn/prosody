...
export default function FeedbackCard({ result, isLoading = false }: FeedbackCardProps) {
  if (isLoading) {
    return (
  ...
  const renderFeedbackItem = (item: FeedbackItem, index: number) => {
    ...
    return (
      <View key={`feedback-${index}`} style={styles.feedbackItem}>
        <Ionicons name={iconMap[item.type] as any} size={20} color={colorMap[item.type]} />
  ...
  return (
    <View style={styles.container}>
      ...
      {result.feedback && result.feedback.length > 0 && (
        <View style={styles.feedbackContainer}>{result.feedback.map(renderFeedbackItem)}</View>
      )}
    </View>
  );
}
